...
public class MultiMailSend {
    ...
    public void sendTextMail(MailSendInfo info) {
        ...
        try {
            ...
            transport.connect("smtp.sina.com", "15234281406@sina.cn", "654321lop");
            //4、创建邮件消息
    ...
    public void sendImageEmail(MailSendInfo info) {
        ...
        try {
            ...
            Transport transport = sendMailSession.getTransport();
            //3、使用邮箱的用户名和密码连上邮件服务器，发送邮件时，发件人需要提交邮箱的用户名和密码给SMTP服务器，用户名和密码都通过验证之后才能够正常发送邮件给收件人。
            //transport.connect("smtp.163.com", "iamsillyboy", "iamsillyboy123");
            transport.connect("smtp.163.com", "iamasillyboy", "654321lop");
            //4、创建邮件消息
    ...
    public void sendMixedMail(MailSendInfo info, ArrayList<String> list) {
        ...
        try {
            ...
            Transport transport = sendMailSession.getTransport();
            //3、使用邮箱的用户名和密码连上邮件服务器，发送邮件时，发件人需要提交邮箱的用户名和密码给SMTP服务器，用户名和密码都通过验证之后才能够正常发送邮件给收件人。
            //transport.connect("smtp.163.com", "iamsillyboy@163.com", "iamsillyboy123");
            //4、创建邮件消息
    ...
    private Message createAttachmentMail(Session sendMailSession, ArrayList<String> list) {
        ...
        try {
            ...
            message.setRecipient(Message.RecipientType.TO, to);
            //邮件标题
            message.setSubject(MimeUtility.encodeText(info.getSubject(),"gb2312","B"));

            // 创建邮件正文，为了避免邮件正文中文乱码问题，需要使用CharSet=UTF-8指明字符编码
            MimeBodyPart text = new MimeBodyPart();
            text.setContent(info.getContent(), "text/html;charset=gb2312");
    ...
    public MimeMessage createSimpleMail(Session sendMailSession) {
        ...
        try {
            ...
            //创建邮件的接受者地址，并设置到邮件消息中
            Address to = new InternetAddress(info.getToAddress());
            Log.e("邮件", info.getToAddress());
            //设置邮件消息的接受者, Message.RecipientType.TO属性表示接收者的类型为TO
            mailMessage.setRecipient(Message.RecipientType.TO, to);
            //设置邮件标题
            mailMessage.setSubject(MimeUtility.encodeText(info.getSubject(),"gb2312","B"));
            //设置邮件内容
            mailMessage.setText(info.getContent(),"gb2312");
            //设置邮件发送的时间
